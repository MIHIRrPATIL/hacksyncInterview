...
const VoiceArena = ({ questions = [], onFinish }: VoiceArenaProps) => {
  ...
  useEffect(() => {
    ...
    const assistant = {
      name: "SkillSphere Interviewer",
      firstMessage: "Hello! I'm your SkillSphere AI interviewer. I've reviewed your coding solution. Let's move on to the voice evaluation questions. Are you ready?",
      model: {
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional technical interviewer at SkillSphere. 
    ...
    vapi.start(assistant as any);

    vapi.on('call-start', () => {
      setIsConnected(true);
      setIsConnecting(false);
      // toaster.create({ title: "Connected to AI", type: "success" });
    });
    ...
    vapi.on('error', (e) => {
      ...
      setIsConnecting(false);
      if (e?.error?.message !== "Call ended") {
        // Suppress toaster for specific errors if needed, or show more friendly message
        // toaster.create({ title: "Voice Connection Error", description: "See console for details.", type: "error" });
      }
    });
    ...
    return () => {
      ...
      initialized.current = false; // Allow re-mount if user comes back
    };
