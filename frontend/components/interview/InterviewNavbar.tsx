...
import { Timer, Radio, Hash } from 'lucide-react';
  ...
  return (
    <nav className={`flex justify-between items-center px-8 py-4 border-b shadow-sm sticky top-0 z-50 transition-colors duration-500 ${isDarkMode ? 'bg-black border-gray-800 text-white' : 'bg-white border-gray-100 text-black'}`}>
      <div className="flex items-center gap-2">
         ...
         <div className={`text-xl font-black tracking-tighter transition-colors ${isDarkMode ? 'text-white' : 'text-black'}`}>SKILLSPHERE</div>
      </div>
      ...
      <div className="flex items-center gap-3 bg-orange-50 px-5 py-2 rounded-2xl border border-orange-100 shadow-sm shadow-orange-100/20">
        <Timer className="w-4 h-4 text-orange-600" />
        <span className="text-xl font-black font-mono text-orange-600 tracking-tighter">
